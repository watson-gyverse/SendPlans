...
function AllProjects() {
  ...
  const q = query(dbRef, where('place', '==', storeName), orderBy('startDate'))
    ...
    <div>
      <PC>
        <Background>
          ...
          <Body>
            <HorizonDiv>
              <div>
                <h2>종료 여부 관계없이 등록한 모든 숙성일지를 표시합니다.</h2>
                <h3>
                  (※ 조회가 안 될 경우, '조회'버튼을 한 번 더 클릭해주세요)
                  <br />
                  (※ 진행 중인 프로젝트는 숙성 후 무게와 종료일이 표시되지
                  않습니다.)
                </h3>
              </div>
      ...
      <Mobile>
        <MobileBackground>
          ...
          <MobileBody>
            <MobileButton onClick={handleButtonClick}>조회하기</MobileButton>
            <h3>
              종료 여부 관계없이 <br />
              등록한 모든 숙성일지를 표시합니다.
            </h3>
    ...
    if (doc.exists) {
      ...
      if (data.afterWeight === 0) {
        setDataArray((arr) => [
          ...
          {
            ...data,
            startDate: new Date(data.startDate.seconds * 1000).toLocaleString(
              'ko-Kr',
              {
                year: 'numeric',
                month: 'numeric',
        ...
        setDataArray((arr) => [
          ...arr,
          {
            ...
            endDate: new Date(data.endDate.seconds * 1000).toLocaleString(
              'ko-Kr',
              {
                year: 'numeric',
                month: 'numeric',
            ...
            id: id,
            loss: (data.beforeWeight - Number(data.afterWeight)).toString(),
            lossP:
              ((data.beforeWeight - Number(data.afterWeight)) * 100) /
              data.beforeWeight,
            finalWeight: data.finalWeight ?? data.afterWeight,
            finalLoss:
              data.beforeWeight - (data.finalWeight ?? data.afterWeight),
            finalLossP:
              ((data.beforeWeight - (data.finalWeight ?? data.afterWeight)) *
                100) /
              data.beforeWeight,
